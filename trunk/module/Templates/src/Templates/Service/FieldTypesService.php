...
class FieldTypesService implements ServiceManagerAwareInterface, EventManagerAwareInterface
{
    ...
    public function getEntityRepository()
    {
        if (null === $this->entityRepository) {
            $this->setEntityRepository($this->getEntityManager()->getRepository('Application\Model\FieldTypes'));
        }
    ...
    public function getList(){
        ...
        $qb
            ->select(array('a.fieldTypeName, a.fieldTypeDescription'))
    ...
    public function getById($id = null){
        ...
        $qb
            ->select(array(
                'a.fieldTypeId', 
                'a.fieldTypeName',
                'a.fieldTypeDescription',
                'a.fieldTypeTemplate', 
                'a.fieldTypeActive', 
                'a.fieldTypeDescription'))
            ->from('Application\Model\FieldTypes', 'a')
            ->where('a.fieldTypeId = :id')
            ->setParameter('id', $id);
    ...
    public function update($data = array()){
        // print_r($data); exit();
        $params = "";
        $params .= "{$data['fieldTypeId']},";
        $params .= "'{$data['fieldTypeName']}',";
        $params .= "'{$data['fieldTypeDescription']}',";
        $params .= "'{$data['fieldTypeTemplate']}',";
        $params .= "1,";
        $params .= "1";
        
        $em = $this->getEntityManager();
        $sth = $em->getConnection()->prepare("CALL FIELD_TYPES_UPDATE({$params});");
        $sth->execute();
    }
    ...
    public function add($data = array()){
        ...
        $em = $this->getEntityManager();
        $sth = $em->getConnection()->prepare("CALL FIELD_TYPES_ADD({$params});");
        $sth->execute();
    }
