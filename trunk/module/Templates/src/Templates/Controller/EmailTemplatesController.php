...
class EmailtemplatesController extends AbstractActionController
{
    ...
    public function addAction(){
        $entityService = $this->getServiceLocator()->get('EmailTemplateService');
        ...
        if ($request->isPost()) {
            ...
            $validatorService->validate("emailTemplateName", "Name", $request->getPost("emailTemplateName"), "Alnum");
            
            if($validatorService->isValid()){
                
                // ... Save Data ...
                 $entityService->add($request->getPost());
                $this->flashMessenger()->addMessage("{$request->getPost('emailTemplateName')} has been successfully added.");
                
                // Redirect to list of main
                $this->redirect()->toUrl('/templates/email_templates');
            } else {
        ...
        return new ViewModel(array(
            'template'          => $data,
            'statuses'          => $entityService->getStatuses(),
    ...
    public function editAction(){
        ...
        if ($request->isPost()) {
            ...
            if($validatorService->isValid()){
                ...
                $this->flashMessenger()->addMessage("{$request->getPost('emailTemplateName')} has been successfully updated.");
    ...
    public function jsonGetListAction(){
        ...
        $data = $entityService->datatables2(array(
            "stored_proc" => "EMAIL_TEMPLATES_SELECT",
